...
for distance in distances:
    ...
    dissociation_curve.append((distance, energy))

# Print the dissociation curve
with open("report.txt", "w") as file:
    print("Dissociation Curve (H-H distance vs. Energy)", file=file)
    print("-------------------------------------------", file=file)
    print("H-H Distance (Å)   Energy (Hartree)", file=file)
    for distance, energy in dissociation_curve:
        print(f"{distance:.2f}               {energy:.6f}", file=file)


x, y = zip(*dissociation_curve)
plt.plot(x, y, marker='o')  # 'o' for circular markers
